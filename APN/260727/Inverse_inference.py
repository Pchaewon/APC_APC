...
CONFIG = {
    ...
    'eqp_col': 'eqp_nm_3200',
    'eqp_groups': {
        # 두 장비군 모두 temp + table_speed + tension 전부 역산 ('all')
        '15-19': {'eqps': ['BSWS38','BSWS42','BSWS44'], 'optimize': 'all'},
...
def inverse_for_target(model, scaler, meta, target_y, base_row,
    ...
    if optimize == 'temp':
        ...
        # temp 12 + table_speed 12 + tension
    ...
    n_ts = len(table_speed_cols)
    def objective(opt_vec):
        x = x_full.copy()
        x[opt_idx] = opt_vec
        loss = (predict(x) - target_y) ** 2
        if lambda_smooth > 0:
            # 프로파일 변수(temp, table_speed)에 smoothness
            if optimize == 'temp':
                loss += lambda_smooth * np.sum(np.diff(opt_vec) ** 2)
            elif optimize == 'table_speed':
                loss += lambda_smooth * np.sum(np.diff(opt_vec) ** 2)
            elif optimize == 'both':
                loss += lambda_smooth * np.sum(np.diff(opt_vec[:n_temp]) ** 2)
            elif optimize == 'all':
                # 앞 12 temp, 다음 12 table_speed
                loss += lambda_smooth * np.sum(np.diff(opt_vec[:n_temp]) ** 2)
                loss += lambda_smooth * np.sum(np.diff(opt_vec[n_temp:n_temp+n_ts]) ** 2)
        return loss
...
def validate(cfg):
    ...
    for grp_name, grp_cfg in cfg['eqp_groups'].items():
        ...
        for eqp in grp_cfg['eqps']:
            ...
            for _, row in sub.iterrows():
                ...
                # 온도 12개 (temp/both/all)
                if optimize in ('temp', 'both', 'all'):
                    for tc in meta['temp_cols']:
                        ro[f'rec_{tc}'] = rec.get(tc)
                        ro[f'actual_{tc}'] = row.get(tc, np.nan)
                # table_speed 12개 (table_speed/all)
                if optimize in ('table_speed', 'all'):
                    for sc in meta.get('table_speed_cols', []):
                        ro[f'rec_{sc}'] = rec.get(sc)
                        ro[f'actual_{sc}'] = row.get(sc, np.nan)
                # tension (tension/both/all)
                if optimize in ('tension', 'both', 'all'):
            ...
            if change_col in sub.columns:
                ...
                if len(g0) > 0 and len(g1) > 0:
                    ...
                    # table_speed 방향
                    if optimize in ('table_speed', 'all'):
                        scol = 'set_table_speed_60pct'
                        if scol in g0.columns:
                            sd = np.sign(g1[scol].mean() - g0[scol].mean())
                            dr['tspeed60_0'] = round(g0[scol].mean(), 3)
                            dr['tspeed60_1'] = round(g1[scol].mean(), 3)
                            dr['tspeed_change_dir'] = int(sd)
                    # tension 방향
                    if optimize in ('tension', 'both', 'all'):
    ...
    if len(num_df) > 0:
        ...
        # table_speed60 역산-실제
        if 'rec_set_table_speed_60pct' in num_df.columns:
            ds = (num_df['rec_set_table_speed_60pct'] -
                  num_df['actual_set_table_speed_60pct']).dropna()
            if len(ds) > 0:
                print(f"  [table_speed_60pct] 역산-실제 차이: "
                      f"mean={ds.mean():+.4f}, |mean|={ds.abs().mean():.4f}")
        # 장비군별로도 분리 출력
