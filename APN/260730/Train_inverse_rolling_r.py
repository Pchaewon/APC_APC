...
CONFIG = {
    ...
    'window': 10,
    'min_runs': 3,
    'include_roll_bow': False,   # ★ roll_BOW 제외 (자기상관 leakage 방지)
    'use_eqp_dummy': True,
    # 장비별 inverse 대상 (학습은 전체, 추천은 이 장비들만)
    'inverse_target_eqps': ['BSWS38', 'BSWS42', 'BSWS44'],
    'inverse_target_bow': 1.75,   # 목표 BOW (스펙 중앙)
    'test_csv': r'D:\chaewon\APC\02.TF\260726\data\test_df.csv',
    'inverse_out_csv': r'./inverse_by_eqp.csv',
    'ridge_alpha': 5.0,
    'split_ratio': 0.8,
    'lambda_smooth': 0.0,
    'encoding':   'utf-8',
...
def build_dataset(cfg):
    ...
    # wire 컬럼 확인
    if cfg['wire_col'] not in df.columns:
        wire_candidates = [c for c in df.columns
                           if 'wire' in c.lower() or 'reel' in c.lower()]
        raise KeyError(
            f"wire_col '{cfg['wire_col']}' 없음. "
            f"후보: {wire_candidates}. CONFIG의 wire_col을 맞는 걸로 바꾸세요.")

    # rolling feature 생성 (BOW + 조건 컬럼 평균)
    roll_cfg = {
        'wire_col': cfg['wire_col'], 'date_col': cfg['date_col'],
        'eqp_col': cfg['eqp_col'],   # ★ 장비+wire 그룹핑
...
def get_feature_sets(cfg):
    ...
    X feature 구성:
      ...
      · 직전 평균: roll_{조건}만  (roll_BOW는 자기상관 leakage라 제외)
      · 장비 더미

    ★ roll_BOW(직전 BOW 평균) 제외 이유:
      진단 결과 roll_BOW 계수 0.83 → "직전 BOW 복사"로 온도 무시(계수 0.6→0.11).
      R²는 0.83으로 높지만 온도 제어력 상실 → inverse 불가.
      roll_조건만 쓰면 R² 0.23, 온도계수 0.52 유지 → inverse 가능.
    """
    temp_cols = cfg['temp_cols']
    include_roll_bow = cfg.get('include_roll_bow', False)   # 기본 제외
    roll_cols = [f'roll_{c}' for c in cfg['roll_source_cols']]
    if include_roll_bow:
        roll_cols = [f'roll_{cfg["target"]}'] + roll_cols
...
def inverse_by_equipment(cfg):
    """
    지정 장비(BSWS38/42/44)에 대해 장비별 온도 recipe 역산.
    · 학습은 전체 데이터 모델 사용 (장비더미 포함)
    · 각 장비의 직전 run 평균(roll_조건)을 조건으로,
      해당 장비 더미를 1로 고정해 온도 역산
    · test 데이터의 각 lot에 대해 수행 → 장비별 결과 CSV

    반환: 장비별 역산 결과 DataFrame
    """
    # 모델·메타 로드
    with open(pt.join(cfg['model_dir'], 'meta.json'), encoding='utf-8') as f:
        meta = json.load(f)
    ...
    # test 데이터에 rolling feature 생성 (직전 평균)
    ...
    EQP = cfg['eqp_col']
    target_bow = cfg['inverse_target_bow']
    target_eqps = cfg['inverse_target_eqps']

    rows = []
    for eqp in target_eqps:
        esub = test_df[test_df[EQP] == eqp].copy()
        # 직전 평균이 계산된 행만 (roll_조건 전부 있어야)
        esub = esub.dropna(subset=roll_cols)
        if len(esub) == 0:
            print(f"  {eqp}: 직전 run 평균 있는 행 없음 — 스킵")
            continue
        print(f"  {eqp}: {len(esub)}개 lot 역산...")

        for _, row in esub.iterrows():
            # 이 lot의 직전 평균 조건
            roll_values = {rc: float(row[rc]) for rc in roll_cols}
            # 현재 온도(있으면 시작점)
            cur_temp = ([float(row[c]) for c in temp_cols]
                        if all(c in row and pd.notna(row[c]) for c in temp_cols)
                        else None)
            r = inverse_current_run(cfg, target_bow, roll_values, eqp,
                                    current_temp=cur_temp)
            out = {'eqp': eqp, 'target_bow': target_bow,
                   'predicted_bow': r['predicted_bow']}
            # 역산 온도 12개
            for c in temp_cols:
                out[f'rec_{c}'] = r['recommended_temp'][c]
                # 실측 온도도 (있으면)
                if c in row and pd.notna(row[c]):
                    out[f'actual_{c}'] = round(float(row[c]), 4)
            # 참고: 직전 BOW 평균, wire
            if f'roll_{cfg["target"]}' in row:
                out['prev_bow_avg'] = round(float(row.get(f'roll_{cfg["target"]}', np.nan)), 4)
    ...
    print(f"\n 저장: {cfg['inverse_out_csv']}")
    print(f"   장비별 역산: {len(res)}개 lot")
    for eqp in target_eqps:
        n = sum(res['eqp'] == eqp) if len(res) else 0
        print(f"     {eqp}: {n}개")
    return res
