...
try:
    ...
    if df_y.empty:
        ...
        # 최종 장비 안전망
        eqp_check = next((c for c in ['EQP_NAME', 'EQP_ID', 'EQP_NM'] if c in df_merged.columns), None)
        if eqp_check:
            before = len(df_merged)
            df_merged = df_merged[df_merged[eqp_check].apply(match_target)].copy()
            print(f"  [최종 장비필터] {eqp_check} 기준 {before} → {len(df_merged)}행")
        df_final = df_merged
    ...
    if df_final is not None and not df_final.empty:
        ...
        eqp_col = next((c for c in ['EQP_NAME', 'EQP_ID', 'EQP_NM'] if c in df_final.columns), None)
        if eqp_col:
            got = set()
            ...
            print(f"  [10대 커버리지] {len(got)}/10: {sorted(got)}")
            if missing:
