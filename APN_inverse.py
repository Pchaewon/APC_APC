...
def inverse_for_target(model, scaler, meta, target_y, base_row,
    ...
    def objective(opt_vec):
        ...
        if optimize == 'temp' and lambda_smooth > 0:
...
def validate(cfg):
        ...
        for eqp in grp_cfg['eqps']:
            ...
            for _, row in sub.iterrows():
                ...
                if optimize == 'tension':
                    ...
                    rec = inv['optimized']
                    ro = {'eqp': eqp, 'group': grp_name, 'optimize': 'temp',
                          'target_bow': round(target_y, 3),
                          'pred_bow': inv['predicted_bow'], 'gap': inv['gap'],
                          change_col: row.get(change_col, np.nan)}
            ...
            if change_col in sub.columns:
                ...
                if len(g0) > 0 and len(g1) > 0:
                    col = tension_col if optimize == 'tension' else 'set_frame_temp_60pct'
                    actual_dir = np.sign(g1[col].mean() - g0[col].mean())
                    bow_dir = np.sign(g1[TARGET].mean() - g0[TARGET].mean())
                    direction_rows.append({
                        'eqp': eqp, 'group': grp_name, 'optimize': optimize,
                        'bow_0': round(g0[TARGET].mean(), 3),
                        'bow_1': round(g1[TARGET].mean(), 3),
                        f'{col}_0': round(g0[col].mean(), 3),
                        f'{col}_1': round(g1[col].mean(), 3),
                        'bow_change_dir': int(bow_dir),
                        'recipe_change_dir': int(actual_dir),
    ...
    if len(num_df) > 0:
        ...
        tp = num_df[num_df['optimize'] == 'tension']
        if len(tp) > 0 and 'diff' in tp.columns:
            vd = tp['diff'].dropna()
            if len(vd) > 0:
                print(f"  [21년식 tension] 역산-실제 차이: "
                      f"mean={vd.mean():+.4f}, |mean|={vd.abs().mean():.4f}, "
                      f"std={vd.std():.4f}")
        tempp = num_df[num_df['optimize'] == 'temp']
        if len(tempp) > 0 and 'rec_set_frame_temp_60pct' in tempp.columns:
            d60 = (tempp['rec_set_frame_temp_60pct'] -
                   tempp['actual_set_frame_temp_60pct']).dropna()
            if len(d60) > 0:
                print(f"  [15-19 temp_60pct] 역산-실제 차이: "
